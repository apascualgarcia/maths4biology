...
xy=[-1.8 1.8 -2 2];
xx=linspace(-1.8,1.8,1000);  ym1=-2*xx;  
...
set(gca,'XTick',[-1 0 1],'YTick',[-1 0 1],'FontSize',20,'FontWeight','bold');

%assign labels for handles
g=legend(h,'f_{1}(x) = -2x','f_{2}(x) = x^{3}','f_{1}(x) + f_{2}(x)',4);
set(g,'Color','none')
%export to eps (change path and file name accordingly)
 print('-depsc2',sprintf('C:\\Documents and Settings\\ajay\\Desktop\\BootCampFig_1203\\BC_2003\\matlab\\fig16b'));
